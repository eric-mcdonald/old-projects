...
void IRCClient::HookIRCCommand(std::string command, IrcCmdCallbackFn function)
{
    IRCCommandHook hook;
...
void IRCClient::UnhookCmd(const std::string &cmd) {
	for (std::list<IRCCommandHook>::iterator it = _hooks.begin(); it != _hooks.end();) {
		if (it->command == cmd) {
			it = _hooks.erase(it);
		}
		else {
