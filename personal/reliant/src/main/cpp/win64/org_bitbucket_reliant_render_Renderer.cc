 ...
 * Class:     org_bitbucket_reliant_render_Renderer
 * Method:    drawTriangle
 * Signature: (FFFFFFIJJIJ)Z
 */
/*JNIEXPORT jboolean JNICALL Java_org_bitbucket_reliant_render_Renderer_drawTriangle
  (JNIEnv *env, jobject this_obj, jfloat x1, jfloat y1, jfloat z1, jfloat base, jfloat height, jfloat w, jint color, jlong d3d_device, jlong vertex_buffer, jint vertex_fvf, jlong texture) {
	if (vertex_fvf & D3DFVF_TEX1) {
		D3dTexVertex quad_vertices[3];
		quad_vertices[0].x = x1;
		quad_vertices[0].y = y1 + height;
		...
		quad_vertices[2].x = x1 + base;
		quad_vertices[2].y = y1;
		quad_vertices[2].z = z1;
		...
		const bool success = d3d_device_ptr->DrawPrimitive(D3DPT_TRIANGLELIST, 0, 1) == D3D_OK;
		if (base_texture != NULL) {
			d3d_device_ptr->SetTexture(0, NULL);
		}
		return success;
	} else {
		D3dColVertex quad_vertices[3];
		quad_vertices[0].x = x1;
		quad_vertices[0].y = y1 + height;
		quad_vertices[0].z = z1;
		const float inverse_w = w == 0.0F ? 1.0F : 1.0F / w;
